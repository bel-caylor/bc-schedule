                    ...
                    <table>
                        <thead class="text-left">
                            <tr class="sticky top-0 left-0">
                                <th class="col-1 sticky top-0 left-0 bg-white z-10">Role</th>
                        ...
                        <tbody>
                            <?php foreach ($events as $group => $group_roles) : ?>
                                ...
                                <!-- colspan="<?php echo (count($all_events) + 1); ?>"  -->
                                    <th scope="colgroup" class="!text-white bg-[#005878] col-1 sticky top-0 left-0 py-2 pl-4 pr-3 text-left text-sm font-semibold sm:pl-3">
                                        <?php echo $group; ?>
                                    </th>
                                    <th colspan="<?php echo (count($all_events)); ?>" class="bg-[#005878]"></th>
                                </tr>
                                <?php foreach ($group_roles as $role => $role_events) : ?>
                                    <tr class="">
                                        <td class="col-1 sticky top-0 left-0 font-semibold bg-white z-10"><?php echo $role; ?></td>
                                        <?php foreach ($all_events as $event) : ?>
                                            <td>
                                                <?php
                            ...
                            <tr class="bg-gray-100">
                                <td class="col-1 sticky top-0 left-0 font-semibold bg-gray-100">Exclude</td>
                                <?php foreach ($all_events as $event) : ?>
                                    <td class="max-w-20">
